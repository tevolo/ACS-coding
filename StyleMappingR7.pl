...
if ($_ =~ m/<span class="image-right">/) {
	...
	$output =~  s/<br \/>//g;
		...
		getstore($url2get, $image2save) or die 'Unable to get page'; #######################################################################
...
my $list = $_[0];
	...
	#   print $elem . "\n";
		$elem =~ m/([A-Za-z0-9]*_\d*?)":{ "Type":"Article","Title":"(.*)?","Web Extension":"([A-Za-z]*)?","WebSiteSection":"PublicWebSite:\d*/;
			...
			$url =~ s/\s+/_/g;
			$url2 = $url;
			$url2 =~ s/,|:|\///g; # this is the replacement for titles should probably add all none safe URL encoding characters
