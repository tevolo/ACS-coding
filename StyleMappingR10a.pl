#!c:/strawberry/perl/bin/perl.exe 
  ...
  use LWP 5.64;
$ArtTitle = "";
my $dir = 'C:\acs\SA\content\acs\global\international\regional';  ## change per env ##
my $path = 'c:\temp\\';
$doccounter = 0;
$pdfcounter = 0;
$othercounter = 0;
...
#### from below <\/h[1-6]>
if ($_ =~ m/<h[1-6]>(.*)/) {
# print " this is from h tag regex \n $_ \n" ;
        ...
        my $paragraph = "";
		 ...
		 ##  print " before $j is j before paramatch is added in get_par\n";
 		 if ($parent and $parent->tag eq 'span') {
 		  # $real_pars = $pars[$j++];
		  $j_prime = $j + $paramatch;
		  $real_pars = $pars[$j_prime];
		   print " now $j is j and $paramatch is para and a is $a and j_prime is $j_prime from get_par\n";
 		 }
 		 $real_pars = $pars[$j_prime];
...
sub GenericImage {
	...
	print " from Generic Image filename is $_[0] and placement is $Placement \n";
		...
		if ($output =~ m/img border="([0-9]+)?"/) {
			push(@Array,'border="' . $_[0] . '"');
		}
		
		if ($output =~ m/height="([0-9]+)"?/) {
			push(@Array,'height="' . $_[0] . '"');
		}
		if ($output =~ m/width="([0-9]+)"?/) {
		push(@Array,'width="' . $_[0] . '"');
		}
