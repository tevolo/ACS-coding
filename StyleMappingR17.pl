#!c:/strawberry/perl/bin/perl.exe 
  ...
  use LWP 5.64;
#####################################################
# R17 add image gallery style for image right left center
#
#
#
$ArtTitle = "";
...
if ($_ =~ m/<span class="image-center">/) {
	...
	&GenericImage($1,"center",$output);
	 #  print "\n  this is filename $filename and output  $output  from image center \n ";	
		}
...
sub GenericImage {
	...
	$Placement = $_[1];
	my $output = $_[2];
	 print " from Generic Image filename is $_[0] and placement is $Placement and outfile is \n $output \n";
	...
	if ($response->is_success) {
		...
		$image2save =~ s/\\/\//g;
			if ($output =~ m/div class="image-caption"\>(.*)?"/) {
